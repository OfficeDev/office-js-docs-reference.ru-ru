...
> **Важно!** автоматически формируется эталонный контент в этом репозитории. Ознакомьтесь с заметками [о документации](https://github.com/OfficeDev/office-js-docs-reference/blob/master/DocumentationToolingNotes.md) перед изменением файлов.
...
С помощью этого метода вы можете вносить изменения в одну статью за раз.
...
**Примечание.** Большую часть сведений из этого раздела можно найти в [справочных статьях GitHub].  Если вы знакомы с Git и GitHub, перейдите к разделу " **участие и редактирование контента** ", в котором описывается характер кода/содержимого этого репозитория.
