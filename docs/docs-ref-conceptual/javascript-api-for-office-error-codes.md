...
|3010|Неподдерживаемая операция привязки|Выбранное содержимое должно быть в формате таблицы. Придайте данным формат таблицы и повторите попытку.|Разработчик пытается применить метод **addRowsAsynch** или **deleteAllDataValuesAsynch** объекта **TableBinding** к данным с типом приведения _"матрица"_.|
...
|12002|||Одно из следующих:<br> – По URL-адресу, переданному в `displayDialogAsync`, не существует страницы.<br> – Страница, переданная в метод `displayDialogAsync`, загружена, но выполнена попытка открыть из диалогового окна страницу, которую не удается найти или загрузить, или для которой указан URL-адрес с недопустимым синтаксисом. Исключение в диалоговое окно и trigges `DialogEventReceived` событий на главной странице.|
|12003|||Диалоговое окно было направлены на URL-адрес для протокола HTTP. HTTPS является обязательным. Исключение в диалоговое окно и trigges `DialogEventReceived` событий на главной странице.|
|12004|||Домен URL-адрес передается `displayDialogAsync` не является надежным. Домен должен быть в одном домене на главной странице (в том числе протокол и номер порта). Возникает при вызове `displayDialogAsync`.|
|12005|||URL-адрес передается `displayDialogAsync` использует протокол HTTP. HTTPS является обязательным. Возникает при вызове `displayDialogAsync`. (В некоторых версиях Office сообщение об ошибке, возвращаемое с 12005 аналогична один возвращаются для 12004.)|
|12006|||Диалоговое окно закрыто. Скорее всего, пользователь нажал кнопку **X**. Исключение в диалоговое окно и trigges `DialogEventReceived` событий на главной странице.|
|12007|||Диалоговое окно с уже открыт из этой главного окна. Главное окно, такие как области задач, может иметь только один диалоговое окно Открыть за раз. Возникает при вызове `displayDialogAsync`.|
|13000 - 13010|||В разделе [причины и обработки ошибок из getAccessTokenAsync](https://docs.microsoft.com/office/dev/add-ins/develop/troubleshoot-sso-in-office-add-ins#causes-and-handling-of-errors-from-getaccesstokenasync).|
...
|Матрица|Текст в ячейке|Создается привязка типа _матрица_ для всей ячейки. В документе не ожидается каких-либо изменений.|
...
|Текст|Выбор нескольких элементов|Для последнего выделенного фрагмента создается оболочка из элемента управления содержимым и привязки к нему. Создается элемент управления содержимым типа _текст_.|
|Текст|Недопустимое выделение (например, несколько объектов, недопустимые объекты и т. д.)|Создать привязку невозможно.|

## <a name="see-also"></a>См. также
   
- [Жизненный цикл разработки надстроек Office](https://docs.microsoft.com/office/dev/add-ins/concepts/add-in-development-lifecycle)
