...
* https://appsforoffice.microsoft.com/lib/beta/hosted/office.js— Используйте этот ресурс при работу функций предварительного просмотра.
...
Надстройки, предназначенных для Word 2016 или более поздней версии выполните, передав функции в метод **Word.run()** . Функции, переданной в метод **запуска** должен быть аргумент контекста. Этот [объект context](/javascript/api/word/word.requestcontext) , отличается от объекта контекста, которые можно получить из объекта Office, но также используется для взаимодействия с среды выполнения Word. Объект context предоставляет доступ к объектной модели Word JavaScript API. Следующем примере показано, как для инициализации и выполнения слово надстройки с помощью метода **Word.run()** .
...
## <a name="word-javascript-api-reference"></a>Справочник по API JavaScript для Word

Получить подробные сведения об интерфейсе API JavaScript для Word обратитесь к [Справочная документация по Word JavaScript API](/javascript/api/word).
