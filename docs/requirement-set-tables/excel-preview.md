...
||[exit()](/javascript/api/excel/excel.namedsheetviewcollection#exit--)|Выходит из активного представления листа.|
...
||[fillEmptyCells](/javascript/api/excel/excel.pivotlayout#fillemptycells)|Указывает, должны ли пустые ячейки в pivotTable быть заполнены с помощью `emptyCellText` .|
...
||[emailsToAssign](/javascript/api/excel/excel.taskchanges#emailstoassign)|Задает адреса электронной почты пользователей, назначаемой задаче.|
